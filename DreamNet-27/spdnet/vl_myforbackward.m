...
for i=1:n
  ...
  switch l.type
    ...
    case 'reconstructionloss'
      if i == 29
          data_recon = res(i-24).x; % 7  8
          gamma = 0.1; % 0.001;
      elseif i == 20
          data_recon = res(i-7).x; 
          gamma = 0.0;
      else
          data_recon = res(i-6).x; % 6  7
          gamma = 0.0;
      end
      bp = 1.0;
      res(i+1).obj = gamma * vl_myreconstructionloss(res(i).x, data_recon, bp); % may be adjusted
      res(i+1).x = res(i).x;
    case 'log'
      res(i+1).x = vl_mylog(res(i).x) ;
    case 'softmaxloss'
      [res(i+1).obj, res(i+1).prob] = vl_mysoftmaxloss(res(i).x, l.class, su) ;
      res(i+1).x = res(i-2).x;
    case 'custom'
...
if doder
  ...
  for i = n:-1:max(1, n-opts.backPropDepth+1) % calculate the derivate in reversed order
    ...
    switch l.type
      ...
      case 'softmaxloss'
        [res(i).dzdx, res(i+1).prob] = vl_mysoftmaxloss(res(i).x, l.class, su, 1) ; % res(i+1).dzdx
      case 'custom'
