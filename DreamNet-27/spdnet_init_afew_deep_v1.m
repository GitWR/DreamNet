...
opts.datadim = [63,53,43,33,43,33,43,33,43]; % the dimensionality of each bimap layer, which also indicates the kernel size of each layer
% 45,25,10
...
for iw = 1 : opts.layernum % designed to initialize each cov kernel 
    ...
    elseif iw==7
        A = rand(opts.datadim(iw));
        [U1, ~, ~] = svd(A * A');
        temp = U1(:,1:opts.datadim(iw+1));
        Winit{iw} = temp;
    else
       A = rand(opts.datadim(iw+1));
       [U1, ~, ~] = svd(A * A');
       temp = U1(:,1:opts.datadim(iw)); % the initialized filters are all satisfy column orthogonality
       Winit{iw} = temp';
    end
end
